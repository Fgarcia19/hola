...
process_execute (const char *file_name)
{
  ...
  exec_name=strtok_r((char*)temp_copy," ",&save);
...
start_process (void *file_name_)
{
  ...
  if (!success)
    {
      update_parent_load_status (t, LOAD_FAILED);
      thread_exit ();
    }

  update_parent_load_status (t, LOAD_SUCCESS);
...
setup_stack (void **esp,const char * file_name)
{
  ...
  if (kpage != NULL)
  {
    ...
    *esp = PHYS_BASE;
    ...
    while((key=strtok_r(file_name," ",&aux)))
    {
      file_name=save;
      args[n_args] = key;
      n_args++;
    ...
    {
      *esp = *esp - sizeof(char) * (strlen(args[i])+1);
...
struct process *
get_child_process (struct thread *t, tid_t child_tid)
{
  if (t == NULL)
    return NULL;
...
void
update_parent (struct thread *child)
{
...
void
update_parent_load_status(struct thread *child, enum load_status status)
{
  struct process *p = get_child_process (child->parent, child->tid);
